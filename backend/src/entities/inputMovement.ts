...
export const buildSchemaInputMovement = () => {
    const builder = loadStruct();

	return builder.getSchema()
}

export const buildStructInputMovement = () => {
    const builder = loadStruct();
...
function loadStruct() {
	const builder = new structBuilder('InputMovement');
	...
	builder.addField({ name: "identification", type: "bool", label: "Identificacao", maxSize: 1, mode: mode.form, pattern: ''});
	builder.addField({ name: "registration", type: "bool", label: "Registro", maxSize: 1, mode: mode.form, pattern: ''});
